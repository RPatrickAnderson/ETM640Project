...
## Introduction ***Review for completeness***
...
## Problem Definition ***Expand and update****
...
##Literature Review  ***Expand and update****
...
## Defining Goals ***Updated 14Mar19**
...
## Goal Programming Model **Outdated requires update**
...
### Parameters  *** Updated 15 Mar 19, Need to add value ranges (*e.g. binary, etc):  
...
\end{split}
$$
## Decision Variable **Updated 15 Mar 19 **
...
\end{split}
$$
# Weights *** NEED TO EXPLAIN WEIGHTS**
1,1,1,2,3
0,1,0,2,0
...
```{r Read Trip Data}
# reads in the instructor data from a csv file.
#NOTE: Must update file PATH to get this to work!!!
...
RowNames <- lapply(list(rep("I",M)),paste0,1:N) # Instructor numbers: I1, I2, ...               
...
#Goals
GT1 <- 10*N # where N is the total number of instructors and 10 is the highest preference rating possible.
GT2 <- 79954.50 # Squadron ProD Target Budget changes year to year based on available funds 
...
model <- MIPModel() %>%
  ...
  # add_constraint((Wm[g]*Dm[1,g]+Wp[g]*Dp[1,g])/GT[g] <= Q, g=1:2)
  #  
  add_constraint((Wm[g]*Dm[d,g]+Wp[g]*Dp[d,g])/GT[g] <= Q, g=1:G,d=1:N)

#****WARNING**** Model sent to give first feasible solution to aid in rapid results for testing code
result <- solve_model(model, with_ROI(solver = "symphony", first_feasible = FALSE))
```

# RESULTS *** NOT UPDATED WORKING ON THIS***
...
# Total Num of instructors going on the t-th trip
NumAssignedToEachTrip <- tabulate(as.numeric(AR[,1]),nbins = NumOfTrips)

# Total Num of top performers going on the ith trip
NumOfTopPerformersAssignedToEachTrip <- matrix(rep(0, each=NumOfTrips), ncol=1)

# Manual counting of all the top performers for each trip.
for(i in 1:nrow(AR))
{
  if(AR[i,2]*InstructorMerit[AR[i,2]] >= 1)
  {
    NumOfTopPerformersAssignedToEachTDY[AR[i,1]] = NumOfTopPerformersAssignedToEachTDY[AR[i,1]] + 1
  }
}

#TripSummary (TDYName, #Assigned, Capacity, MeritTrip? AssignedOnMerit  Cost/Person)
TripSummaryChart <- cbind(TripNames,NumAssignedToEachTrip,SA,
                          NumOfTopPerformersAssignedToEachTrip,TM,C) 
colnames(TripSummaryChart)<-list("Names", "Total", "Cap",
                                 "NumOfTPs", "GloryTrip?", "Cost/Per")

# Total Num of instructors going on the ith trip
NumTripsAssignedToEachInstructor <- tabulate(as.numeric(AR[,2]),nbins = N)

# Blank vector to be build up 
NumOfMeritTripsForEachInstructor<- matrix(rep(0, each=N), ncol=1)

# Of the trips each instructor went on how many trips had merit based classification?
for(t in 1:nrow(AR))
{
  if(AR[t,1]*TripMeritStatus[AR[t,1]] >= 1)
  {
    NumOfMeritTripsForEachInstructor[AR[t,2]] = NumOfMeritTripsForEachInstructor[AR[t,2]] + 1
  }
}
...
for(k in 1:nrow(AR))
{
  ...
  if ((AssignmentByInstructorMatrix[AR[k,2],3] == 0) && (AssignmentByInstructorMatrix[AR[k,2],2] != 0) && (AssignmentByInstructorMatrix[AR[k,2],1] != 0))
  {
    AssignmentByInstructorMatrix[AR[k,2],3] = AR[k,1]
  }
    #if first slot is filled then use the second slot, unless already filled
  if((AssignmentByInstructorMatrix[AR[k,2],2] == 0) && (AssignmentByInstructorMatrix[AR[k,2],1] != 0))
  {
    AssignmentByInstructorMatrix[AR[k,2],2] = AR[k,1]
    }
    #if first trip slot isn't filled then record trip, otherwise use next slot
  if(AssignmentByInstructorMatrix[AR[k,2],1] == 0)
  {
    AssignmentByInstructorMatrix[AR[k,2],1] = AR[k,1]
     }
 }

InstructorNumber <- matrix(c(1:N),ncol=1)
...
InstructorChart <- cbind(InstructorNumber,NumTripsAssignedToEachInstructor,NumOfMeritTripsForEachInstructor,
                         IM,S,AssignmentByInstructorMatrix)
...
# GoalDataVectors
G2 <- matrix(c(TotalSpent,ProDTripBudgetTarget,BDminusResult,BDpostiveResult),nrow=1)
